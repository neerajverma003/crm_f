...
const CreateCustomer = () => {
  ...
  const [activeTab, setActiveTab] = useState("create"); // "create" or "transfer"
  ...
  useEffect(() => {
    if (editModalOpen && selectedLead) {
      setEditForm({
        ...
        whatsAppNo: selectedLead.whatsAppNo || "",
    ...
    <div className="p-6 max-w-8xl mx-auto bg-white shadow rounded">
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form className="space-y-4">
              ...
              <div className="mt-4 flex justify-end">
                <button onClick={() => setViewModalOpen(false)} className="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 transition">Close</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {typeof editModalOpen !== 'undefined' && editModalOpen && selectedLead && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">Edit Transfer Lead</h3>
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                const res = await fetch(`http://localhost:4000/employeelead/transfer/${selectedLead._id}`, {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Name</label>
                  <input type="text" value={editForm.name || ''} onChange={(e)=>setEditForm(prev=>({...prev,name:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Email</label>
                  <input type="email" value={editForm.email || ''} onChange={(e)=>setEditForm(prev=>({...prev,email:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Phone</label>
                  <input type="text" value={editForm.phone || ''} onChange={(e)=>setEditForm(prev=>({...prev,phone:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">WhatsApp</label>
                  <input type="text" value={editForm.whatsAppNo || ''} onChange={(e)=>setEditForm(prev=>({...prev,whatsAppNo:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Destination</label>
                  <input type="text" value={editForm.destination || ''} onChange={(e)=>setEditForm(prev=>({...prev,destination:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Departure City</label>
                  <input type="text" value={editForm.departureCity || ''} onChange={(e)=>setEditForm(prev=>({...prev,departureCity:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700">Places To Cover (comma separated)</label>
                <input type="text" value={(editForm.placesToCoverArray && editForm.placesToCoverArray.join(', ')) || editForm.placesToCover || ''} onChange={(e)=>setEditForm(prev=>({...prev,placesToCoverArray:e.target.value.split(',').map(p=>p.trim())}))} className="w-full border p-2 rounded" />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Group Number</label>
                  <input type="text" value={editForm.groupNumber || ''} onChange={(e)=>setEditForm(prev=>({...prev,groupNumber:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">No. of Days</label>
                  <input type="text" value={editForm.noOfDays || ''} onChange={(e)=>setEditForm(prev=>({...prev,noOfDays:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Lead Status</label>
                  <input type="text" value={editForm.leadStatus || ''} onChange={(e)=>setEditForm(prev=>({...prev,leadStatus:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Lead Source</label>
                  <input type="text" value={editForm.leadSource || ''} onChange={(e)=>setEditForm(prev=>({...prev,leadSource:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Lead Type</label>
                  <input type="text" value={editForm.leadType || ''} onChange={(e)=>setEditForm(prev=>({...prev,leadType:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700">Trip Type</label>
                <input type="text" value={editForm.tripType || ''} onChange={(e)=>setEditForm(prev=>({...prev,tripType:e.target.value}))} className="w-full border p-2 rounded" />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">Child Ages (comma separated)</label>
                  <input type="text" value={(editForm.childAges && editForm.childAges.join(', ')) || ''} onChange={(e)=>setEditForm(prev=>({...prev,childAges:e.target.value.split(',').map(v=>v.trim()).filter(Boolean)}))} className="w-full border p-2 rounded" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Expected Travel Date</label>
                  <input type="date" value={editForm.expectedTravelDate || ''} onChange={(e)=>setEditForm(prev=>({...prev,expectedTravelDate:e.target.value}))} className="w-full border p-2 rounded" />
                </div>
              </div>

              <div className="flex justify-end gap-2">
                <button type="button" onClick={()=>setEditModalOpen(false)} className="bg-gray-600 text-white px-4 py-2 rounded">Cancel</button>
