...
//             setEmployeeData((prev) => prev.map((att) => (att._id === updatedAttendance.attendance._id ? updatedAttendance.attendance : att)));
...
//                     <h2 className="mb-4 text-center text-lg font-semibold">Edit Attendance</h2>
...
//                     if (recordDate === today && employeeId) {
...
//             console.log("📊 Today's Attendance Map:", attendanceById);
...
//         // Fetch monthly attendance for this user
//         await fetchMonthlyAttendance(user);
//     };

//     /* -------------------------------------------------------------------------- */
//     /* 📊 Fetch Monthly Attendance Data */
//     /* -------------------------------------------------------------------------- */
//     const fetchMonthlyAttendance = async (user) => {
//         try {
//             let attendanceData = [];
...
//         }
//         return user?.department?.departmentName || "N/A";
//     };
...
//                         </button>
//                         <button
//                             onClick={() => setEditingAttendance(user)}
...
//                     <td className="px-4 py-3">
//                         <span className={`rounded-full px-3 py-1 text-xs font-semibold bg-blue-100 text-blue-700`}>{designation}</span>
//                     </td>
...
//                                                 const stats = calculateAttendanceStats(monthlyAttendance, currentMonth);
//                                                 // const monthlyBaseSalary = 30000; // Static monthly salary
//                                                 const monthlyBaseSalary = selectedUser?.salary|| 0;
//                                                 const perDaySalary = monthlyBaseSalary / 30; // Assuming 30 working days per month
//                                                 const presentDays = stats.present + stats.gracePresent; // Count present and grace present as working days
//                                                 const totalEarned = perDaySalary * presentDays;
...
//             const presentDays = stats.present + stats.gracePresent;
...
//                                                             <label className="mb-2 block text-xs font-medium text-gray-700">Salary Status</label>
//                                                             <select
//                                                                 value={salaryStatus}
//                                                                 onChange={(e) => setSalaryStatus(e.target.value)}
//                                                                 className="w-full rounded border border-purple-300 bg-white px-3 py-2 text-sm font-medium text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500"
//                                                             >
...
//                                                             className="w-full rounded-lg bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-2 text-sm font-medium text-white shadow-md transition-all duration-200 hover:from-blue-600 hover:to-blue-700 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50"
//                                                         >
    ...
    const MonthlyAttendanceGrid = ({ attendanceData, month }) => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            days.push(
                <div
                    key={i}
                    className={`cursor-pointer rounded-lg border-2 p-2 text-xs transition-all duration-200 hover:shadow-lg active:scale-95 sm:rounded-xl sm:p-3 sm:text-sm md:p-4 md:text-base md:hover:scale-105 ${getStatusColor(status)}`}
                    onDoubleClick={() => handleCardInteraction(date, dayRecords)}
                    onClick={() => handleCardInteraction(date, dayRecords)}
                    title="Double-click or double-tap to edit"
                >
                    <div className="space-y-1 sm:space-y-2 md:space-y-3">
                        {/* Date Number */}
                        <div className="flex items-center justify-center">
                            <p className="text-lg font-bold text-gray-900 sm:text-xl md:text-2xl">{i}</p>
                        </div>
                        ...
                        {dayRecords && dayRecords.length > 0 ? (
                            <div className="space-y-1 sm:space-y-1.5 md:space-y-2">
                                ...
                                {/* Status Badge */}
                                <div className="flex justify-center border-t border-gray-300 border-opacity-40 pt-1 sm:pt-1.5">
                                    <span
                                        className={`rounded-full px-2 py-0.5 text-xs font-bold sm:px-2.5 sm:py-0.5 md:px-3 md:py-1 ${getStatusBadgeColor(status)}`}
                                    >
                                        {status || "No Status"}
                                    </span>
                                </div>
                            ...
                            <div className="py-1 text-center sm:py-1.5">
                                <span className="inline-block rounded-full bg-gray-300 px-1.5 py-0.5 text-xs font-semibold text-gray-700">
                                    No Data
                                </span>
                            </div>
                        )}
        ...
        return (
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 sm:gap-2.5 md:grid-cols-7 md:gap-3">
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-6">
                                ...
                                >
                                    ...
                                    <div className="space-y-2 p-3 text-xs">
                                        ...
                                        {attendance ? (
                                            ...
                                            <div className="flex items-center justify-center rounded bg-gray-50 py-2">
                                                <span className="text-gray-500">No attendance data</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
            ...
            {viewModal && selectedUser && (
                <div className="animate-in fade-in fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md duration-300">
                    <div className="animate-in zoom-in relative flex h-screen w-screen flex-col overflow-y-auto rounded-none border-0 bg-white shadow-2xl duration-300">
                        <div className="sticky top-0 z-50 flex items-center justify-between border-b border-gray-200 bg-white p-3 sm:p-6">
                            <h2 className="text-lg font-bold text-gray-900 sm:text-2xl md:text-3xl">Attendance Details</h2>
                            <div className="flex items-center gap-2 sm:gap-4">
                                <button
                                    onClick={handlePrint}
                                    className="hidden items-center gap-2 rounded-lg bg-gradient-to-r from-blue-500 to-indigo-600 px-3 py-1.5 text-sm font-medium text-white shadow-md transition-all duration-200 hover:from-blue-600 hover:to-indigo-700 hover:shadow-lg sm:flex"
                                    title="Print attendance details"
                                >
                                    <Printer size={18} />
                                    Print
                                ...
                                <button
                                    onClick={handlePrint}
                                    className="flex items-center rounded-lg bg-gradient-to-r from-blue-500 to-indigo-600 p-1.5 text-white shadow-md transition-all duration-200 hover:from-blue-600 hover:to-indigo-700 hover:shadow-lg sm:hidden"
                                    title="Print attendance details"
                                >
                                    <Printer size={18} />
                                </button>
                            ...
                            <div className="mb-4 sm:mb-6 md:mb-8">
                                ...
                                <div className="mb-3 sm:mb-4 md:mb-6">
                                    <div className="flex items-start gap-2 sm:gap-3 md:gap-4">
                                        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-400 to-indigo-600 text-lg font-bold text-white sm:h-14 sm:w-14 sm:text-xl md:h-16 md:w-16 md:text-2xl">
                                            {selectedUser?.fullName?.charAt(0).toUpperCase()}
                                        </div>
                                        <div className="min-w-0">
                                            ...
                                            <p className="mt-0.5 text-xs text-gray-600 sm:text-sm">{selectedUser?.userType}</p>
                                        </div>
                                    </div>
                                </div>

                                {/* Employee Info Grid (2 columns) */}
                                <div className="grid grid-cols-1 gap-3 rounded-lg bg-gray-50 p-3 sm:grid-cols-2 sm:gap-4 sm:p-4 md:gap-6 md:p-6">
                                    {/* Contact Information */}
                                    <div>
                                        <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-700">Contact Information</h3>
                                        <div className="space-y-2">
                            ...
                            <div className="border-t border-gray-200 pt-4 sm:pt-6 md:pt-8">
                                <div className="grid grid-cols-1 gap-4 md:grid-cols-4 md:gap-8">
                                    ...
                                    <div className="md:col-span-3">
                                        {/* Month & Year Selection */}
                                        <div className="mb-4 sm:mb-6 md:mb-8">
                                            <div className="mb-3 flex flex-col items-center gap-2 sm:mb-4 sm:gap-3 md:mb-6 md:gap-4">
                                                <h3 className="text-center text-2xl font-bold text-gray-900 sm:text-3xl md:text-4xl">
                                                    {currentMonth.toLocaleDateString("en-IN", { month: "long", year: "numeric" })}
                                                </h3>

                                                <div className="flex flex-col items-center justify-center gap-2 sm:gap-3 md:flex-row md:gap-4">
                                                    {/* Month Selector */}
                                                    <select
                                                        value={currentMonth.getMonth()}
                                                        onChange={(e) =>
                                                        ...
                                                        className="w-full rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 sm:text-sm md:w-auto md:px-4 md:py-2"
                                                    >
                                                        <option value="0">January</option>
                                                        <option value="1">February</option>
                                                        <option value="2">March</option>
                                            ...
                                            <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2 md:gap-3">
                                                <button
                                                    onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1))}
                                                    className="rounded-lg bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-200 sm:px-3 sm:py-2 sm:text-sm md:px-4 md:py-2"
                                                >
                                                    ← Prev
                                                </button>
                                                <button
                                                    onClick={() => setCurrentMonth(new Date())}
                                                    className="rounded-lg bg-blue-100 px-2.5 py-1.5 text-xs font-medium text-blue-700 transition-colors hover:bg-blue-200 sm:px-3 sm:py-2 sm:text-sm md:px-4 md:py-2"
                                                >
                                                    Today
                                                </button>
                                                <button
                                    ...
                                    {/* RIGHT - Attendance & Salary Summary (Stacked) */}
                                    <div className="col-span-1 flex h-fit flex-col gap-3 sm:gap-4 md:sticky md:top-8 md:gap-6">
                                        {/* Attendance Summary */}
                                        <div className="rounded-lg border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-3 sm:p-4 md:p-6">
                                            <h3 className="mb-1 text-sm font-bold text-gray-900 sm:mb-2 sm:text-base md:mb-2 md:text-lg">
                                                Attendance Summary
                                            </h3>
                                            <p className="mb-3 text-xs font-medium text-gray-600 sm:mb-4 md:mb-6">
                                                {currentMonth.toLocaleDateString("en-IN", { month: "long", year: "numeric" })}
                                            </p>
